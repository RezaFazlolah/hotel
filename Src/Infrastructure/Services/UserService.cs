    ...
    : BaseService<Guid, User>(context), IUserService
{
    ...
    public virtual Task<Result<ICollection<Reservation>>> GetReservationsAsync(Guid userId,
    ...
    public override async Task<Result<User>> GetByIdAsync(Guid id, CancellationToken ct)
    {
        try
        {
            ...
            return result is null ? Result<User>.Failure(new Error($"{EntityName} with id {id} not found", ErrorCode.NotFound), ResultCode.NotFound) : Result<User>.Success(result);
        }
        catch
        {
